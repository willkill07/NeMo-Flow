...
|---|---|---|---|
| CLI coding-agent proxy | A local coding-agent harness owns invocation and provider routing | [CLI Basic Usage](/nemo-relay-cli/basic-usage) | The enrolled agent runs normally and emits hook plus managed provider lifecycle output. |
| Persistent coding-agent installs | You want the maintained install path for Codex, Claude Code/Desktop, or Hermes Agent | [Coding Agent Installation](/nemo-relay-cli/plugin-installation) | `nemo-relay doctor <agent>` confirms the enrolled integration is ready. |
...
Enroll a local coding agent in the persistent per-user proxy, verify
fail-closed hooks and provider routing, and export traces or trajectories.
</Card>
