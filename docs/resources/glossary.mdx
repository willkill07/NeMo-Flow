...
authenticated per-user Relay proxy.
...
: Model Context Protocol (MCP) is an open protocol for model clients to discover
  and invoke external tools or context servers. NeMo Relay coding-agent
  enrollment does not use MCP to manage proxy lifecycle.
...
: A persistent installation is a Relay-owned coding-agent integration created
  with `nemo-relay install <agent>`. It registers the agent with one persistent
  authenticated proxy for the OS user, patches user configuration, installs
  trusted hooks, and records an immutable generation fence. The proxy endpoint
  is discovered and persisted rather than fixed machine-wide.
...
**Relay Gateway**
: The Relay gateway is the managed execution engine used for provider-facing
  LLM traffic. In coding-agent integrations, a persistent per-user proxy invokes
  that engine directly for authenticated HTTP, SSE, and WebSocket traffic.
