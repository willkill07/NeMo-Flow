...
Run the repository-only real-service compatibility test from the NeMo Relay
repository root:
...
The test builds Relay's separate `internal-test-server` feature-gated binary.
That binary is absent from normal release builds and is not a supported
standalone deployment path. The test starts these local processes:

| Process | Port | Readiness Check or Purpose |
| --- | --- | --- |
| Switchyard Decision API | `4000` | Serves `/health`, routing decisions, and ATOF ingestion. |
| NeMo Relay test harness | `4041` | Serves `/healthz` and OpenAI-compatible test traffic. |
