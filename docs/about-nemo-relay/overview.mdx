...
work? The answer determines whether you should use the local coding-agent proxy, direct SDK
...
| Goal | Start With | Why |
|---|---|---|
| Observe Codex, Claude Code, or Hermes locally | [NeMo Relay CLI](/nemo-relay-cli/about) and [Basic Usage](/nemo-relay-cli/basic-usage) | Relay enrolls the client in one authenticated per-user proxy, forwards hooks, manages supported native provider traffic, and writes observability artifacts without changing application code. |
