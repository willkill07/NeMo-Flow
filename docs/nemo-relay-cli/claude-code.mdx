...
Use this guide to observe Claude Code sessions with NeMo Relay. The transparent
wrapper and direct marketplace installation on this page target the terminal
CLI. To wrap both terminal Claude Code and the Claude Desktop Code tab through
the same persistent, fail-closed service, refer to [Claude
Desktop](/nemo-relay-cli/claude-desktop). Claude Chat, Cowork, and web traffic
remain outside that integration's scope.
