...
description: "Install and manage persistent NeMo Relay integrations for Claude Desktop, Claude Code, Codex, and Hermes Agent."
position: 3
---
...
Claude Desktop is a separate, explicit integration target because it installs
current-user TLS trust and a login service in addition to reusing the Claude
Code plugin:
...
It wraps both the Desktop Code tab and bare terminal Claude Code while
preserving Claude subscription OAuth. It is intentionally excluded from
`install all` and `uninstall all`. Refer to [Claude
Desktop](/nemo-relay-cli/claude-desktop) for its platform requirements,
certificate policy, proxy allowlist, and release-gate status.
...
nemo-relay install claude-desktop  # explicit; never selected by `all`
```
...
Claude Desktop diagnosis remains explicit: use `doctor --plugin
claude-desktop`. It is not selected by `doctor --plugin all`, because its
trusted TLS material and login-service lifecycle are independently
release-gated.
...
Claude Desktop uninstall restores its field-level Claude settings patch,
removes the exact generated current-user root and login service, and returns a
pre-existing Claude Code plugin to direct-gateway mode. Like installation,
Desktop is not selected by `uninstall all`.
