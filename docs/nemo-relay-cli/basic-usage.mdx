...
Claude Desktop is explicit because it adds current-user TLS trust and a login
service. It is not selected by `all`:

```bash
nemo-relay install claude-desktop
nemo-relay claude-desktop --folder .
```

Refer to [Claude Desktop](/nemo-relay-cli/claude-desktop) for the Code-tab
traffic policy and platform release gates.
