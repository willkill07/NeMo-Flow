...
After `plugins add` registers the worker, run
`nemo-relay plugins edit --user`. In the user plugin file, add the
configuration table to the `[[plugins.dynamic]]` entry that `plugins add`
created. The configuration sets the tag that the worker adds to tool request
JSON:
...
To test runtime validation in the persistent coding-agent proxy:

1. Register the worker in the user plugin file.
2. Set `reject = true`, then force-refresh an enrolled coding agent. Relay
   starts the worker and reports the worker diagnostic before it installs
   registrations.
3. Set `tag` to a non-string value, then force-refresh again. Relay reports the
...
Register and enable the worker in user policy, validate it, and refresh an
enrolled agent:

```bash
nemo-relay plugins enable examples.python_grpc_worker
nemo-relay plugins validate examples.python_grpc_worker
nemo-relay install codex --force
# Linux only: export CODEX_CA_CERTIFICATE="<path printed by install>"
nemo-relay doctor codex
```

Run Codex normally to exercise the configured component. The removed
standalone `--bind` launch path is not a worker-validation mechanism.
